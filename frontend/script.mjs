...
function triggerInput() {
  ...
  try {
    ...
    ast = parser.parse(proofInput.value);
    const result = Verifier.verifyFromAST(ast);
    let resultHtml = result.message;

    if (!result.valid) {
      resultHtml += `<p>Step: ${result.errorStep}`;
      if (result.errorSrcLine) {
        resultHtml += `, Src Line: ${result.errorSrcLoc.first_line}`;
      }
      resultHtml += `</p>`;
...
parentheses.addEventListener("change", triggerInput);
proofInput.addEventListener("input", triggerInput);

let active_example_id = "9";
...
function onExampleClick() {
  if (active_example_id) {
    document.getElementById(active_example_id).classList.remove("active");
  }
  this.classList.add("active");
  const id = this.id;
  active_example_id = id;
  proofInput.value = examples.examples[id].join("\n");
  proofInput.dispatchEvent(new Event("input"));
}

proofInput.setAttribute(
  "style",
  "height:" + proofInput.scrollHeight + "px;overflow-y:hidden;",
);
proofInput.addEventListener("input", () => {
  proofInput.style.height = "auto";
  proofInput.style.height = proofInput.scrollHeight + "px";
});
...
proofInput.value = examples.examples[9].join("\n");
proofInput.dispatchEvent(new Event("input"));
