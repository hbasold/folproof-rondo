...
describe("Justifier Tests", function () {
  ...
  it("should accept numbers and number ranges", function () {
    const j = new Justifier(
      {
        hasPart: false,
        stepRefs: ["num", "range", "num"],
        subst: false,
      },
      dummyFn,
    );
    const proof = Verifier.preprocess(
      parser.parse("a: sorry\n| b: sorry\n| c: sorry\nd\ne"),
    );
    const msg = j.checkParams(proof, 4, null, ["1", "2-3", "4"], null);
    assert.deepEqual(msg, [null, [0, [1, 2], 3], null]);
  ...
  it("should reject out-of-range numbers", function () {
    ...
    let msg = j.checkParams(proof, 1, null, ["1", "1-1"], null);
    assert.deepEqual(msg, [null, [0, [0, 0]], null]);

    msg = j.checkParams(proof, 1, null, ["2", "1-1"], null);
    assert.equal(
      typeof msg,
      "string",
    ...
    msg = j.checkParams(proof, 1, null, ["1", "2-2"], null);
    assert.equal(typeof msg, "string", "Should reject ranges >= current step.");

    msg = j.checkParams(proof, 1, null, ["1", "3-2"], null);
